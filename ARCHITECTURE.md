...
[`cli/commands/session-tab.ts`](cli/commands/session-tab.ts) persists a `--group` → tab-id map to `~/.interceptor/session-tab.json`, mirroring the `~/.interceptor/<subsystem>/state.json` convention used by the iOS surface (`daemon/ios/state.ts`) — a global file rather than per-PID state, since the CLI is a fresh process per invocation. `interceptor tab designate [id]` writes it (resolving to the most recently opened interceptor-managed tab when no id is given, via `resolveMostRecentTab` in `cli/commands/tabs.ts`); `interceptor tab self` reads it back; `interceptor read` with no `--tab` falls back to it through `resolveReadTargetTabId` in `cli/commands/compound.ts`. Like the per-group auto-target keys described above, the designation is scoped by `--group`/`INTERCEPTOR_GROUP` (the group in effect is read from `getGlobalGroup()` in `cli/transport.ts`): each group holds an independent slot inside the one file, so concurrent agents in different groups never clobber each other's designated tab, while invocations with no group share a single default slot. A legacy flat `{ tabId }` file (written before scoping) is still read back as that default slot.
