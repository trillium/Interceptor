...
- [interaction-skills/canvas-camera-overlays.md](canvas-camera-overlays.md) — earlier sibling pattern with the projection helper, URL-watcher, disposable handler pattern, and antimeridian wrap.
- [interaction-skills/canvas-rendered-editor-input.md](canvas-rendered-editor-input.md) — companion pattern for canvas-rendered TEXT editors when you need to write content rather than navigate a camera.
- [interaction-skills/blob-export-capture.md](blob-export-capture.md) — capture client-side render output without Save dialogs.
